...
Zero Tokenは切り替えを決して隠しません。フォールバックが作動すると、エージェントは無料プロバイダー上で動作します——メインモデルではありません。各プロバイダーには独自のプライバシーポリシーとデータ保持期間があります。**機密データを送信する前に各プロバイダーの規約を確認してください。** セットアップスクリプトは設定されたプロバイダーを明確に表示します。あなたがキーを管理します。

> 💡 これは透明で、あなたが制御するフェイルオーバーです。キーはあなたが持ち、プロバイダーはあなたが決めます。
